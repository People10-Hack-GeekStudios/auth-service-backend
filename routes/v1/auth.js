...
const { isPhone, isPhoneOTP, isPass, isDeliveryLogin } = require("./middlewares/validator");
...
// User middleware
const { createUserIfNotExist, validateUserExist, validateUserByID, validateDeliveryUser, validateDeliveryUserLogin, validateShopUserByID, removeGuestFCMFromToken, maxDeviceCheck, validateAdminUserWithPass, checkIsBalanceRemaining } = require("./middlewares/user");
...
router.post('/generate/delivery/user', verifyAccessToken, mongoDBping, isPass, validateShopUserByID, checkIsBalanceRemaining, async (req, res) => {

  req.temp_user.save()
    .then(data => {
      return res.status(200).json({ "response_code": 200, "message": resp["delivery_agent_added"], "response": null });
    })
    .catch(err => {
      console.log(err);
      return res.status(200).json({ "response_code": 500, "message": resp[500], "response": null });
...
router.post('/login/delivery', isDeliveryLogin, mongoDBping, validateDeliveryUserLogin, createTokens, async(req, res) => {
...
router.get('/token/refresh/delivery', verifyRefreshToken, mongoDBping, validateDeliveryUser, refreshTokens, async (req, res) => {
...
router.get('/logout/delivery', verifyAccessToken, mongoDBping, validateDeliveryUser, uncacheToken, async (req, res) => {

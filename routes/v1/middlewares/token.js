...
function refreshTokens(req, res, next) {
  ...
  try {
    ...
    if (req.temp_user.device) {
      ...
      if(!found){ return res.status(402).json({ "response_code": 402, "message": resp["invalid-token"], "response" : null }); }
      else {
        if(req.temp_user.device[pos].token!=req.oldtoken){
          return res.status(402).json({ "response_code": 402, "message": resp["invalid-token"], "response" : null });
        }
      }
    }
...
function verifyRefreshToken(req, res, next) {
  ...
  jwt.verify(token, process.env.REFRESH_TOKEN_SECRET, (err, user) => {
    if (err) return res.status(402).json({ "response_code": 402, "message": resp["invalid-token"], "response" : null })
    if (dateDifference(Date.now(), token.lastRefreshed) > 30) return res.status(402).json({ "response_code": 402, "message": resp["expired-token"], "response" : null })
    req.token = user
    next()
  })
}
